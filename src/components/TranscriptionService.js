  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (selectedFeature === 'audioUrl') {
            ...
            response = await axios.post('http://127.0.0.1:8001/api/test/whisper-remote/', requestData);
        } else if (selectedFeature === 'localAudio') {
            const formData = new FormData();
            formData.append('audio_file', file);
            formData.append('user_prompt', userPrompt);
            response = await axios.post(
                'http://127.0.0.1:8001/api/test/whisper-local/',
                formData,
                {
                    headers: {
            ...
            response = await axios.post('http://127.0.0.1:8001/api/test/youtube/', requestData);
        }
